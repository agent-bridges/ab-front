...
export const mobileEntryDisplayTitle = (entry: MobileEntry) =>
    ...
    <div className="flex h-full min-h-0 flex-col bg-canvas-bg text-canvas-text" data-mobile-workspace>
      <header className="relative z-[60] flex h-10 shrink-0 items-center gap-2 border-b border-canvas-border bg-canvas-surface px-3">
        ...
        <div className="relative min-w-0 shrink">
          <button onClick={() => setAgentMenuOpen((value) => !value)} className="flex max-w-40 items-center gap-1 rounded border border-canvas-border bg-canvas-bg px-2 py-1 text-xs" title={currentAgent ? agentDisplayLabel(currentAgent, daemonDisplayName(currentAgent)) : 'Select agent'}>
            <span className="truncate">{currentAgent ? agentDisplayLabel(currentAgent, daemonDisplayName(currentAgent)) : 'Select'}</span><span className="text-canvas-muted">▾</span>
          </button>
          {agentMenuOpen && <>
              ...
              {relays.map((relay) => <div key={relay.id} className={!relay.enabled ? 'opacity-60' : ''}>
                ...
                {filterOfflineMachines(relay.machines, showOffline).map((machine) => {
                  ...
                  return <div key={agent.id} className={`flex w-full items-center text-xs ${agent.id === currentAgentId ? 'bg-canvas-accent/10 text-canvas-accent' : ''} ${!machine.online ? 'opacity-60' : ''}`}>
                    <button disabled={!relayCanConnect(relay) || !machine.online} onClick={() => { setCurrentAgent(agent.id); setAgentMenuOpen(false); }} className="flex min-w-0 flex-1 items-center gap-2 px-5 py-2 text-left hover:bg-canvas-border disabled:cursor-default"><span className={`h-1.5 w-1.5 rounded-full ${machine.online ? 'bg-emerald-500' : 'bg-canvas-muted'}`} /><span className="min-w-0 flex-1 truncate">{daemonDisplayName(agent)}</span>{!machine.online && <span className="text-[9px]">offline</span>}</button>
                    ...
                    <button className="mr-2 rounded p-1 hover:bg-canvas-border" onClick={() => { setAliasTarget({ kind: 'daemon', agent }); setAgentMenuOpen(false); }} title={`Rename ${agent.name}`}><Pencil size={12} /></button>
                  </div>;
                })}
