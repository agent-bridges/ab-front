        ...
        {favourites.length === 0
          ...
          : favourites.map((item) => {
            ...
            return <div key={`${item.agent.id}:${item.session.id}`} className="group flex min-h-7 items-center gap-2 px-3 text-xs hover:bg-canvas-border">
              <button className="flex min-w-0 flex-1 items-center gap-2 py-1 text-left" onClick={() => onOpen(item)} title={`${sessionDisplayName(item.session)} — ${daemonDisplayName(item.agent)} via ${item.agent.relay_name}`}>
                <WorkspaceEntryIcon entry={entry} />
                <span className="min-w-0 flex-1 truncate">{sessionDisplayName(item.session)}</span>
                <span className="max-w-24 truncate text-[9px] text-canvas-muted">{daemonDisplayName(item.agent)} · {item.agent.relay_name}</span>
              </button>
        ...
        {favourites.map((item) => {
          ...
          return <div key={`${item.agent.id}:${item.session.id}`} className="flex min-w-0 items-center gap-2 rounded-lg border border-canvas-border bg-canvas-surface p-2">
            <button className="flex min-w-0 flex-1 items-center gap-2 text-left" onClick={() => onOpen(item)}>
              ...
              <span className="min-w-0 flex-1"><span className="block truncate text-xs font-medium">{sessionDisplayName(item.session)}</span><span className="block truncate text-[10px] text-canvas-muted">{daemonDisplayName(item.agent)} · {item.agent.relay_name}</span></span>
            </button>
