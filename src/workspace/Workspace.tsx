...
import { createPty, killPty, renameDaemon, setPtyLabel } from '../api/pty';
...
export default function Workspace() {
  ...
  const setServerSessionLabel = usePtyStore((state) => state.setSessionLabel);
  ...
  const [sidebarSection, setSidebarSection] = useState<'relays' | 'fav'>('relays');
  ...
  const entries = useMemo<WorkspaceEntry[]>(() => {
    ...
    return combined.sort((a, b) => {
      ...
      return workspaceEntryDisplayTitle(a).localeCompare(workspaceEntryDisplayTitle(b), undefined, { sensitivity: 'base' });
    });
  ...
  const sidebar = (
    <aside className={`${isMobile ? 'fixed inset-y-10 left-0 z-50 shadow-2xl' : 'relative'} flex min-h-0 flex-col border-r border-canvas-border bg-canvas-surface`} style={{ width: isMobile ? 'min(88vw, 340px)' : sidebarWidth }}>
      <div className="flex h-9 shrink-0 border-b border-canvas-border" role="tablist" aria-label="Sidebar sections">
        <button role="tab" aria-selected={sidebarSection === 'relays'} onClick={() => setSidebarSection('relays')} className={`flex min-w-0 flex-1 items-center justify-center gap-1.5 border-b-2 px-2 text-[11px] ${sidebarSection === 'relays' ? 'border-canvas-accent text-canvas-accent' : 'border-transparent text-canvas-muted hover:bg-canvas-border'}`}><TerminalIcon size={12} />Relays</button>
      ...
      {sidebarSection === 'relays' && <div className="flex h-9 shrink-0 items-center gap-2 border-b border-canvas-border px-2">
      ...
      <div className="min-h-0 flex-1">
        {sidebarSection === 'relays' ? <div className="h-full overflow-y-auto py-1">
        {relays.map((relay) => <div key={relay.id} className={!relay.enabled ? 'opacity-60' : ''}>
          ...
          return <div key={agent.id} className={DESKTOP_TREE_DEPTH_CLASSES.daemonBranch}>
            ...
            <button disabled={!relayCanConnect(relay) || !machine.online} className="flex min-w-0 flex-1 items-center gap-1 text-left text-xs hover:bg-canvas-border disabled:cursor-default"
              ...
              {expanded ? <ChevronDown size={12} /> : <ChevronRight size={12} />}<span className={`h-1.5 w-1.5 rounded-full ${current && connected ? 'bg-green-400' : machine.online ? 'bg-emerald-500/70' : 'bg-canvas-muted'}`} /><span className="truncate font-medium">{daemonDisplayName(agent)}</span>{!machine.online && <span className="ml-auto text-[9px] text-canvas-muted">offline</span>}
            </button>
            ...
            <button className="hidden shrink-0 rounded p-1 group-hover:block hover:bg-canvas-border" onClick={() => setAliasTarget({ kind: 'daemon', agent })} title={`Rename ${agent.name}`}><Pencil size={10} /></button>
            </div>
        ...
        </div> : <FavouritesPanel compact favourites={favourites} loading={favouritesLoading} failedAgents={favouriteFailedAgents} onOpen={openFavourite} onRemove={(item) => void setFavourite(item.agent.id, item.session, false).catch(console.error)} />}
      </div>
  ...
  return <div className="flex h-full min-h-0 flex-col bg-canvas-bg text-canvas-text">
        ...
        {relays.map((relay) => <optgroup key={relay.id} label={`${relay.name} — ${relayStateLabel(relay)}`}>
          {relay.machines.length === 0
            ...
            : relay.machines.map((machine) => {
              const agent = agentById.get(machine.id);
              return <option key={machine.id} value={machine.id} disabled={!relayCanConnect(relay) || !machine.online}>{agent ? agentDisplayLabel(agent, daemonDisplayName(agent)) : machine.name}{!machine.online ? ' — offline' : ''}</option>;
            })}
        </optgroup>)}
      </select>
    ...
    <ClientAliasDialog
      ...
      alias={aliasTarget?.kind === 'daemon' ? aliasTarget.agent.name : aliasTarget?.entry.session.label || ''}
      onSave={async (value) => {
        if (aliasTarget?.kind === 'daemon') {
          const name = value.trim();
          if (!name) throw new Error('Daemon name must not be empty');
          await renameDaemon(aliasTarget.agent.id, name);
          setAgentName(aliasTarget.agent.id, name);
        } else if (aliasTarget) {
          await setPtyLabel(aliasTarget.entry.agentId, aliasTarget.entry.session.id, value);
          setServerSessionLabel(aliasTarget.entry.session.id, value.trim());
        }
      }}
